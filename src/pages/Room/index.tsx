...
export function Room() {
  ...
  useEffect(() => {
    if (user?.id === authorIdRoom){
      if (theme === 'dark') {
        toast('You are the admin of the room, we give you superpowers!', {
          duration: 4000,
          icon: '😉',
          style: {
            borderRadius: '10px',
    ...
    <div id="page-room" className={`${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <header className={`${theme === 'dark' ? 'dark' : ''}`}>
        <div className="content" >
          <svg className={`${theme === 'dark' ? 'dark' : ''}`} width="157" height="75" viewBox="0 0 157 75" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <main className={`${theme === 'dark' ? 'dark' : ''}`}>
        ...
        <form onSubmit={handleSendQuestion}>
          ...
          <div className={`form-footer ${theme === 'dark' ? 'dark' : ''}`}>
            {user ? (
              ...
              <span>Para enviar uma pergunta, <button type="button" onClick={() => handleSignIn()} >faça seu login</button>.</span>
            )}
        ...
        <div className={`question-list ${questions.length <= 0 ? 'empty' : ''}`}>
          {questions.length > 0 ? (
            ...
            user ? (
              <EmptyQuestion content="Seja a primeira pessoa a fazer uma pergunta!" />
            ) : (
              <EmptyQuestion content="Faça o seu login e seja a primeira pessoa a fazer uma pergunta!" />
            )
          )}
        </div>
      </main>
